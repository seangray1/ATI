...
Channyn Rowe</description>
    ...
    <rules>
        ...
        <active>false</active>
        <description>Outbound message to send Opportunity fields to Timberline via Jitterbit</description>
        <formula>AND(OR(AND(ISCHANGED(Stage__c),ISPICKVAL(Stage__c, &quot;Won&quot;)) ,Sync_to_Timberline__c = TRUE, ISCHANGED(PM_Approval_Only__c), AND(OR(ISCHANGED(Project_Manager_new__c), ISCHANGED(Project_Manager__c),ISCHANGED(Account_Executive_1_Timber__c),ISCHANGED(Account_Executive_2_Timber__c),ISCHANGED(Job_Name__c)),NOT(ISBLANK( Last_Successful_Timberline_Update__c)), X1sub__c = FALSE)), 
...
$Profile.Name &lt;&gt; &quot;Restricted Process Execution&quot;)</formula>
    ...
    <rules>
        ...
        <active>false</active>
        <description>Updates Last Timberline Update field with current date time when a record is saved.</description>
        <formula>AND(OR(AND(ISCHANGED(Stage__c),ISPICKVAL(Stage__c, &quot;Won&quot;)) ,Sync_to_Timberline__c = TRUE, ISCHANGED(PM_Approval_Only__c), AND(OR(ISCHANGED(Project_Manager_new__c), ISCHANGED(Project_Manager__c),ISCHANGED(Account_Executive_1_Timber__c),ISCHANGED(Account_Executive_2_Timber__c),ISCHANGED(Job_Name__c)),NOT(ISNULL( Last_Successful_Timberline_Update__c)), X1sub__c = FALSE)), 
